  ...
  return (
    <section className="overflow-hidden rounded-3xl border border-violet-200 bg-gradient-to-br from-violet-50 via-white to-blue-50 p-5 shadow-sm">
      ...
      {!result ? <p className="mt-3 text-sm leading-6 text-slate-600">Review consistency, running, strength, knee response, and the single most useful focus for next week.</p> : <>
        ...
        {result.patterns.length ? <div className="mt-4"><h3 className="text-sm font-bold text-slate-950">{isCurrentWeek ? "What remains" : "Patterns worth noticing"}</h3><ul className="mt-2 space-y-2">{result.patterns.map((item, i) => <li key={i} className="rounded-xl bg-white/80 p-3 text-sm text-slate-700">{item.text}</li>)}</ul></div> : null}
        <div className="mt-4 rounded-2xl bg-violet-700 p-4 text-white"><p className="text-[11px] font-bold uppercase tracking-wider text-violet-100">{isCurrentWeek ? "Focus for the rest of the week" : "One focus for next week"}</p><p className="mt-1 text-sm font-semibold leading-5">{result.nextWeekFocus.text}</p></div>
